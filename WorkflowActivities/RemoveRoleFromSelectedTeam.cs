...
{
    public class RemoveRoleFromSelectedTeam : CodeActivity
    {
        [RequiredArgument]
        [Input("Tean")]
        ...
        protected override void Execute(CodeActivityContext executionContext)
        {
            ...
            if (teamRootRoles.Any())
            {
                ...
                tracingService.Trace("Removing Role from the Team");

                service.Disassociate("team",
                    teamRef.Id,
                    new Relationship("teamroles_association"),
                    new EntityReferenceCollection() { role.ToEntityReference() }
                    );

                tracingService.Trace("Role has removed from the Team");
            }
        }
    }
}
